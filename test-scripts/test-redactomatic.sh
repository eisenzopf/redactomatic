...
OUTPUT=$( { python3 $BINDIR/redactomatic.py $VERBOSE_OPT --no-defaultrules --regextest --no-redact --testoutputfile $regex_test; } 2>&1)
if echo "$OUTPUT" | grep -q "No rulesfiles loaded"; then
  echo 'PASS. Correctly detected no rules being loaded.'
else
  echo 'FAIL. Redactomatic failed to detect missing rules files. Instead it returned:' $OUTPUT
fi

#Test the automatic regular expression testing.
OUTPUT=$(python3 $BINDIR/redactomatic.py $VERBOSE_OPT --regextest --no-redact --testoutputfile $regex_test)

#Test redaction and then anonymization at level 2
python3 $BINDIR/redactomatic.py $VERBOSE_OPT --column 4 --idcolumn 1 --modality voice --rulefile $CUSTOMRULES --chunksize 20 --inputfile $INPUTDIR/$sample_voice --outputfile $voice_redacted_l2 --log $voice_log_l2 --level 2 
python3 $BINDIR/redactomatic.py $VERBOSE_OPT --column 4 --idcolumn 1 --modality text  --rulefile $CUSTOMRULES --inputfile $INPUTDIR/$sample_text --outputfile $text_redacted_l2 --log $text_log_l2 --level 2
python3 $BINDIR/redactomatic.py $VERBOSE_OPT --column 4 --idcolumn 1 --modality voice --rulefile $CUSTOMRULES --inputfile $INPUTDIR/$sample_voice --outputfile $voice_redact_anonymized_l2 --anonymize --seed 1 --level 2
python3 $BINDIR/redactomatic.py $VERBOSE_OPT --column 4 --idcolumn 1 --modality text  --rulefile $CUSTOMRULES --inputfile $INPUTDIR/$sample_text --outputfile $text_redact_anonymized_l2 --anonymize --seed 1 --level 2

#Test redaction at level 3
python3 $BINDIR/redactomatic.py $VERBOSE_OPT --column 4 --idcolumn 1 --modality voice --rulefile $CUSTOMRULES --inputfile $INPUTDIR/$sample_voice --outputfile $voice_redacted_l3 --log $voice_log_l3 --level 3
python3 $BINDIR/redactomatic.py $VERBOSE_OPT --column 4 --idcolumn 1 --modality text  --rulefile $CUSTOMRULES --inputfile $INPUTDIR/$sample_text --outputfile $text_redacted_l3 --log $text_log_l3 --level 3

#Test redaction at level 4
python3 $BINDIR/redactomatic.py $VERBOSE_OPT --column 4 --idcolumn 1 --modality voice --rulefile $CUSTOMRULES --inputfile $INPUTDIR/$sample_voice --outputfile $voice_redacted_l4 --log $voice_log_l4 --level 4
python3 $BINDIR/redactomatic.py $VERBOSE_OPT --column 4 --idcolumn 1 --modality text  --rulefile $CUSTOMRULES --inputfile $INPUTDIR/$sample_text --outputfile $text_redacted_l4 --log $text_log_l4 --level 4
...
python3 compare-files.py $voice_log_l4 $TESTEXPECTED/$voice_log_l4 'Is the L4 voice redaction log correct?'
